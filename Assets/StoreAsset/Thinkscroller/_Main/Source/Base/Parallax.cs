...
///}
		...
		public static Vector2 GetRawScrollVector(ScrollLayer layer, Vector2 scrollValue)
		{
			...
			{	
				...
				{
					return instance.GetParallaxCamera().transform.right * -scrollValue.x * instance.baseSpeed * layer.GetScrollSpeed() * layer.GetScrollMod().x;
				}
				else if (instance.GetScrollConstraints() == ScrollConstraints.Y)
				{
					return instance.GetParallaxCamera().transform.up * -scrollValue.y * instance.baseSpeed * layer.GetScrollSpeed() * layer.GetScrollMod().y;
				}
				else
				{
					...
					return Vector2.Scale(sV * instance.baseSpeed * layer.GetScrollSpeed(), layer.GetScrollMod());
				}
			}
			else
			{
				return Vector2.Scale(scrollValue * instance.baseSpeed * layer.GetScrollSpeed(), layer.GetScrollMod());
			}
		}
...
///}
				...
				if (scrollLayer.enabled)
				{
					if (scrollLayer.isObjectLayer)
					{
						...
						{	
							...
							yy = GetPixelHeight(cam, scrollLayer.cachedTransform.position) * hh;
						}
						
						if (scrollConstraints == ScrollConstraints.X)
						{
							scrollLayer.cachedTransform.position += cam.transform.right * xx * -scrollValue.x * baseSpeed * scrollLayer.GetScrollSpeed() * scrollLayer.GetScrollMod().x;
						}
						else if (scrollConstraints == ScrollConstraints.Y)
						{
							scrollLayer.cachedTransform.position += cam.transform.up * yy * -scrollValue.y * baseSpeed * scrollLayer.GetScrollSpeed() * scrollLayer.GetScrollMod().y;				
						}
						else
						{
							Vector3 sV = (cam.transform.right * xx * -scrollValue.x) + (cam.transform.up * yy * -scrollValue.y);
							scrollLayer.cachedTransform.position += Vector3.Scale(sV, scrollLayer.GetScrollMod()) * baseSpeed * scrollLayer.GetScrollSpeed();								
						}
					}
					else if (scrollLayer.GetMaterial())
					{
						foreach (string textureName in scrollLayer.GetTextureNames())
						{
							...
							if (scrollLayer.GetMaterial().HasProperty(textureName))
							{
								if (scrollConstraints == ScrollConstraints.X)
								{
									scrollLayer.GetMaterial().SetTextureOffset(textureName, WrapVector(scrollLayer.GetMaterial().GetTextureOffset(textureName) + new Vector2(scrollValue.x, 0) * baseSpeed * scrollLayer.GetScrollSpeed() * scrollLayer.GetScrollMod().x));
								}
								else if (scrollConstraints == ScrollConstraints.Y)
								{
									scrollLayer.GetMaterial().SetTextureOffset(textureName, WrapVector(scrollLayer.GetMaterial().GetTextureOffset(textureName) + new Vector2(0, scrollValue.y) * baseSpeed * scrollLayer.GetScrollSpeed() * scrollLayer.GetScrollMod().y));
								}
								else
								{
									scrollLayer.GetMaterial().SetTextureOffset(textureName, WrapVector(scrollLayer.GetMaterial().GetTextureOffset(textureName) + Vector2.Scale(scrollValue, scrollLayer.GetScrollMod()) * baseSpeed * scrollLayer.GetScrollSpeed()));				
								}
							}
						}
					}
		...
		/// <summary>
		/// Gets the height of a pixel at a world space position, relative to a camera.
		/// </summary>
		internal static float GetPixelHeight(Camera camera, Vector3 position)
		{
			//Get the screen coordinate of some point
			var screenPos = camera.WorldToScreenPoint(position);
			var offset = Vector3.zero;
