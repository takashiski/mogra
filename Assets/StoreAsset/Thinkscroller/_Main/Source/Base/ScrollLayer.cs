...
namespace ThinksquirrelSoftware.Thinkscroller
{
...
#endif	
	{
		...
		[SerializeField] [HideInInspector] Vector2 scrollMod = Vector2.one;
		...
		/// </summary>
		public void SetScrollMod(Vector2 scrollMod)
		{
		...
		private void RefreshVertices_INTERNAL()
		{
			...
			float pixel_w = Parallax.GetPixelWidth(parallaxCam, this.cachedTransform.position);
			float pixel_h = Parallax.GetPixelHeight(parallaxCam, this.cachedTransform.position);
			Vector2 offset = this.offset;
			offset.Scale(new Vector2(pixel_w, pixel_h));
			
			switch(alignment)
			{
			case ScrollLayerAlignment.TopLeft:
					offset -= new Vector2(-Parallax.instance.GetCameraWidth() * .5f * pixel_w,
								Parallax.instance.GetCameraHeight() * .5f * pixel_h);
					break;
			
			case ScrollLayerAlignment.Top:
					offset -= new Vector2(0, Parallax.instance.GetCameraHeight() * .5f * pixel_h);
					break;
			...
			case ScrollLayerAlignment.Left:
					offset -= new Vector2(-Parallax.instance.GetCameraWidth() * .5f * pixel_w, 0);
					break;
			...
			case ScrollLayerAlignment.BottomRight:
					offset -= new Vector2(Parallax.instance.GetCameraWidth() * .5f * pixel_w,
								-Parallax.instance.GetCameraHeightRaw() * .5f * pixel_h);
					break;
			}
